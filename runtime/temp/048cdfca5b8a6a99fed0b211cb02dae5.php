<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:90:"D:\project\factory\public/../application/admin\view\epwarehouse\material_detail_lists.html";i:1587716400;s:67:"D:\project\factory\application\admin\view\common\mobile_layout.html";i:1587535506;}*/ ?>
<!DOCTYPE html>
...
<body>
    ...
    <div class="canvas">
        ...
        <script>    $(function(){        warehousetable.start();        select.select();    })</script><head>    <script type="text/javascript" src="<?php echo ADMIN_STYLE_URL; ?>js/warehousetable.js"></script></head><div class="canvas_title do-clear">    <ul class="tab_btn tab_btn_fl fl"><!--        <li><a href="<?php echo url('/admin/warehouse/materialDetailDo','action=Add'); ?>">添加</a></li>-->         <li><a href="javaScript:materialdetail.deleteAll('<?php echo url('/admin/epwarehouse/materialDetailDel'); ?>')">删除</a></li>         <li><a href="javaScript:materialdetail.updateAll('<?php echo url('/admin/epwarehouse/adddisLists'); ?>')">添加到配货单</a></li>        <li><a href="<?php echo url('/admin/epwarehouse/disLists','action=index'); ?>" target="_blank" >查看配货单</a></li><!--        <li><a href="<?php echo url('/admin/warehouse/exportExcel'); ?>" download='物料.xlsx' target="_blank">导出Excel数据</a></li>-->                 <li>           <select id="status">               <option value="5" <?php if($status == 5): ?>selected<?php endif; ?>>是否计划物资</option>               <option value="0" <?php if($status == 0): ?>selected<?php endif; ?>>非计划物资</option>               <option value="1" <?php if($status == 1): ?>selected<?php endif; ?>>计划物资</option>           </select>         </li>        <li>            <select id="neworder" name="neworder">                <option value="5" <?php if($neworder == 5): ?>selected<?php endif; ?>>是否有新订单</option>                <option value="0" <?php if($neworder == 0): ?>selected<?php endif; ?>>无新订单</option>                <option value="1" <?php if($neworder == 1): ?>selected<?php endif; ?>>有新订单</option>            </select>        </li>        <li>            <select id="factory_pid" name="factory_pid">                <option value="5" <?php if($factory_pid == 'all'): ?>selected<?php endif; ?>>全部仓库</option>                <option value="0" <?php if($factory_pid==0): ?>selected<?php endif; ?>>成品区</option>                <option value="3" <?php if($factory_pid==3): ?>selected<?php endif; ?>>FBA</option>                <option value="1" <?php if($factory_pid==1): ?>selected<?php endif; ?>>半成品区域</option>                <option value="2" <?php if($factory_pid==2): ?>selected<?php endif; ?>>瑕疵品区域</option>            </select>        </li>        <li>            <select id="factory_sid" name="factory_sid">                <option value="1" <?php if($factory_sid==1): ?>selected<?php endif; ?>>免费区</option>                <option value="2" <?php if($factory_sid==2): ?>selected<?php endif; ?>>付费区</option>            </select>        </li>        <li class="input"><input type="text" id="search" name="search" value=""       onkeydown="keyFun.key(event,13,search.search,[$('#search'),'<?php echo url('/admin/epwarehouse/materialDetailLists'); ?>'])"      placeholder="搜索：sku" /></li>       <li><a href="javaScript:search.searchArray(['<?php echo url('/admin/epwarehouse/materialDetailLists'); ?>','search','status','neworder','factory_pid','factory_sid'])">搜索</a></li>    </ul>    <?php echo $pageDiv; ?></div><div class="canvas_intro">    <table class="productli">        <thead>            <tr>                <th width="10" class="center">#</th>                <th width="10" class="center"><input name="" type="checkbox" value="" id="select" /></th>                <th width="140">图片</th>                <th width="140">sku</th>                <th width="50">规格</th>                <th width="50">库位</th>                <th width="50">区域</th>                <th width="50">新订单</th>                <th width="50">FBA计划物资</th>                <th width="50" class="center">库存</th>                <th width="50" class="center">入库时间</th>                <th width="50" class="center">生产时间</th>                <th class="center" width="200">操作</th>            </tr>        </thead>        <tbody>            <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$value): $mod = ($k % 2 );++$k;?>            <tr>                <td class="center"><?php echo $k; ?></td>                <td class="center"><input name="select" type="checkbox" value="<?php echo $value['id']; ?>" /></td>                <td><img src='http://kjds-img.img-cn-shanghai.aliyuncs.com/<?php echo explode(',',$value['ep_img'])[0]; ?>@0e_0o_1l_500h_500w.src' /></td>                <td><?php echo $value['sku']; ?></td>                <td><?php echo $value['spec']; ?></td>                <td><?php echo $value['wt_name']; ?></td>                <td><?php if($value['factory_pid'] == 0): ?>                成品区域<?php if($value['factory_sid']==1): ?> 免费区<?php else: ?>付费区<?php endif; elseif($value['factory_pid'] == 1): ?>                    半成品区域                <?php elseif($value['factory_pid'] == 2): ?>                    瑕疵区域                    <?php elseif($value['factory_pid'] ==3): ?>                    FBA                    <?php endif; ?>                </td>                <td><span style="background: #5bb75b;color: #fff;font-size: 12px;padding: 0 10px;"><?php if(!empty($value['orderFactory'])): ?>有<?php else: ?>无<?php endif; ?></span></td>                <td><?php if($value['status'] ==0): ?>否<?php else: ?>是<?php endif; ?></td>                <td class="center"><?php echo $value['count']; ?>件</td>                <td class="center"><?php echo $value['create_time']; ?></td>                <td class="center"><?php if($value['produce_time'] !=''): ?><?php echo $value['produce_time']; else: ?>未设置<?php endif; ?></td>                <td class="center operation"><!--                    <a href="<?php echo url('/admin/epwarehouse/viewMaterial','id='.$value['id']); ?>">查看sku</a>-->                                        <a href="javaScript:materialdetail.update('<?php echo $value['id']; ?>','<?php echo url('/admin/epwarehouse/adddisLists'); ?>')">添加到配货单</a>                    <a <?php if($value['status']=='2'): else: ?>href="javaScript:materialdetail.delete('<?php echo $value['id']; ?>','<?php echo url('/admin/epwarehouse/materialDetailDel'); ?>')"<?php endif; ?>>删除</a>                                   </td>            </tr>            <?php endforeach; endif; else: echo "" ;endif; ?>        </tbody>    </table></div><div class="canvas_title do-clear">    <ul class="tab_btn tab_btn_fl fl">    </ul>    <?php echo $pageDiv; ?></div>
